...
Route::prefix('v1')->group(function (): void {
    ...
    Route::middleware('auth:sanctum')->group(function (): void {
        ...
        Route::post('devices/controls/batch', [DeviceControlController::class, 'batch'])
            ->middleware('throttle:controls');
        Route::get('devices/{device}', [DeviceController::class, 'show']);
        Route::patch('devices/{device}', [DeviceController::class, 'update']);

        Route::get('devices/{device}/users', [DeviceAccessController::class, 'index']);
        Route::post('devices/{device}/users', [DeviceAccessController::class, 'store'])
            ->middleware('throttle:users');
        Route::delete('devices/{device}/users/{access}', [DeviceAccessController::class, 'destroy'])
            ->middleware('throttle:users');

        Route::get('devices/{device}/controls', [DeviceControlController::class, 'index']);
        Route::post('devices/{device}/controls', [DeviceControlController::class, 'store'])
            ->middleware('throttle:controls');
